...
def pick_rate(agent, map):
    statement = "SELECT * FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') AND `map` = '{}';"
    output = cursor.execute(statement.format(agent, agent, agent, agent, agent, map))
    total = cursor.execute("SELECT * FROM vctdata.stage3 WHERE `map` = '" + map + "';")
...
def win_rate(agent, map):
    statement = "SELECT SUM(`win`) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') AND `map` = '{}';"
    cursor.execute(statement.format(agent, agent, agent, agent, agent, map))
    output = cursor.fetchall()
    for x in output:
        output = x[0]
    total = "SELECT COUNT(id) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') AND `map` = '{}';"
...
def attack_win_rate(agent, map):
    statement = "SELECT (sum(`attack wins`) / (sum(`attack wins`) + sum(`attack losses`)) * 100) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') AND `map` = '{}';"
    cursor.execute(statement.format(agent, agent, agent, agent, agent, map))
    output = cursor.fetchall()
    for x in output:
        output = x[0]
...
def defense_win_rate(agent, map):
    statement = "SELECT (sum(`defense wins`) / (sum(`defense wins`) + sum(`defense losses`)) * 100) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') AND `map` = '{}';"
    cursor.execute(statement.format(agent, agent, agent, agent, agent, map))
    output = cursor.fetchall()
    for x in output:
        output = x[0]
...
def team_comp_rankings():
    ...
    statement = "SELECT COUNT(`id`) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
    ...
    for i in list(comb):
        ...
        for x in outputstat:
            outputstat = x[0]
        win_statement = "SELECT COUNT(`id`) FROM vctdata.stage3 WHERE (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
                        AND (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
                        AND (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
                        AND (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
                        AND (`agent 1` = '{}' OR `agent 2` = '{}' or `agent 3` = '{}' or `agent 4` = '{}' or `agent 5` = '{}') \
        ...
        for x in win_total_stat:
            win_total_stat = x[0]
        total = cursor.execute("SELECT COUNT(`id`) FROM vctdata.stage3 WHERE `map` = '" + response + "';")
...
def team_comp_rankings_all_maps():
    ...
    for i in list(comb):
        ...
        for x in win_total_stat:
            win_total_stat = x[0]
        total = cursor.execute("SELECT COUNT(`id`) FROM vctdata.stage3;")
        totalstat = cursor.fetchall()
        for x in totalstat:
            totalstat = x[0]
        final_pick_rate = round(((outputstat / totalstat) * 100), 1)
